...
def fetch(start_date, end_date):
    ...
    while current_date <= end_date:
        ...
        url = f'https://www.espn.com/soccer/scoreboard/_/date/{fecha}'
...
def get_max_date_from_mysql(engine):
    with engine.connect() as connection:
        query = text("SELECT MAX(Date) FROM results WHERE Result IS NOT NULL and date<=CURRENT_DATE()") #football_results
...
def delete_records_after_max_date(engine, max_date):
    with engine.connect() as connection:
        delete_query = text("DELETE FROM results WHERE Date >= :max_date")
