...
def get_max_date_from_mysql(engine):
    with engine.connect() as connection:
        query = text("SELECT MAX(Date) FROM football_results_espn_us WHERE Result IS NOT NULL")
...
def delete_records_after_max_date(engine, max_date):
    with engine.connect() as connection:
        delete_query = text("DELETE FROM football_results_espn_us WHERE Date >= :max_date")
